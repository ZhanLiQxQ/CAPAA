        ...
        for stealth_loss in cfg.stealth_losses:
            for d_thr in cfg.d_threshes:
                for classifier_name in cfg.classifier_names:
                    ...
                    if attacker_name == 'SPAA':
                        cam_infer_adv_untar, prj_adv_untar, warped_prj_untar = spaa(model, classifier, imagenet_labels, [true_idx], targeted_attack, cam_scene, d_thr, stealth_loss, cfg.device, setup_info, fine_grid,cfg.attention_use)
                    elif attacker_name == 'PerC-AL+CompenNet++':
                    ...
                    if attacker_name == 'SPAA':
                        ...
                        cam_infer_adv_tar, prj_adv_tar, warped_prj_tar = spaa(model, classifier, imagenet_labels, target_idx, targeted_attack, cam_scene, d_thr, stealth_loss, cfg.device, setup_info, fine_grid,cfg.attention_use)
                    elif attacker_name == 'PerC-AL+CompenNet++':
                        print(f'\n[ Targeted ] attacking [{classifier_name}], target: ({imagenet_labels[target_idx[v]]})...')
...
def get_attacker_cfg(attacker_name, data_root, setup_list, device_ids=[0], load_pretrained=False, plot_on=True, attention_use = False):
    ...
    # cfg_default.d_threshes       = [2, 3, 5, 7]
    # cfg_default.d_threshes = [5, 6, 7, 8]
    cfg_default.d_threshes = [2, 3, 4, 5]
    # cfg_default.d_threshes = [11, 9, 4, 3]
...
def spaa(pcnet, classifier, imagenet_labels, target_idx, targeted, cam_scene, d_thr, stealth_loss, device, setup_info, fine_grid = None, attention_use = False):
    """
    ...
    raw_score, p, idx = classifier(cam_scene, cp_sz)
    pred_idx = idx[0][0].tolist()
    '''
    ...
    pred_idx = idx[0][0].tolist()
    # 获取当前物体的CAM
    grayscale_cam = cam(input_tensor=trans_cam_scene_batch, target_category = pred_idx) # (1 * 240 *320)
    ...
    # ine_grid 是一个维度为 (batch_size, height, width, 2) 的张量，fine_grid 中的值是 (x, y) 坐标，
    # 范围通常是在 (-1, 1) 之间，代表着归一化的图像坐标。表示对应于输入图像中每个位置的输出图像中的目标位置。
    # 在 (x, y) 坐标归一化范围内，(-1, -1) 代表输出图像的左上角，(1, 1) 代表输出图像的右下角，(0, 0) 代表输出图像的中心。
    # 这种归一化范围的设置使得变形网格可以与不同大小的图像一起使用，同时保持一致的坐标范围。
    ...
    # TODO:prj_CAM_attention =  prj_CAM_attention[0,:]
    # prj_CAM_attention =  prj_CAM_attention[0,:] * 2
    ...
    for i in range(0, iters):
        ...
        if i % 30 == 0 or i == iters - 1:
            ...
            # ut.save_imgs(expand_4d(cam_infer_best), '/mnt/data/zhan/SPAA-main/SPAA-main/data/setups/basketball/debug/cam_infer/' + classifier.name,idx = n)
            # ut.save_imgs(expand_4d(prj_adv_best), '/mnt/data/zhan/SPAA-main/SPAA-main/data/setups/basketball/debug/prj_adv/' + classifier.name,idx = n)
            # n += 1
    # clamp to [0, 1]
...
def attack_results_change_pose(ret, t, imgnet_labels, im_gray, prj_adv, warped_prj, cam_scene, cam_infer, cam_real, prj_im_sz, cp_sz,attacked_names = None):
    ...
    with torch.no_grad():
        ...
        # 如果不是original SPAA或MPAA的universal统计
        if attacked_names == None:
            ...
            im = insert_text(im,f'{imgnet_labels[ret["real"][2][t, 0]]} ({ret["real"][1][t, 0]:.2f})', (540, 14), 14)
        else:
            ...
            for attacked_name in attacked_names:
                i += 1
                im = insert_text(im,
                                 f'{attacked_name + " : "+ imgnet_labels[ret["scene" + attacked_name][2][0, 0]]} (p={ret["scene" + attacked_name][1][0, 0]:.2f})',
                                 (5, 14 * i), 13)
                im = insert_text(im,
                                 f'{attacked_name + " : "+ imgnet_labels[ret["real" + attacked_name][2][t, 0]]} (p={ret["real" + attacked_name][1][t, 0]:.2f})',
                                 (530, 14 * i), 13)
...
def attack_results(ret, t, imgnet_labels, im_gray, prj_adv, warped_prj, cam_scene, cam_infer, cam_real, prj_im_sz, cp_sz,attacked_names = None):
    ...
    with torch.no_grad():
        ...
        if attacked_names == None:
            ...
            im = expand_boarder(im, (0, 26, 0, 0))
            im = insert_text(im,f'{imgnet_labels[ret["scene"][2][0, 0]]} ({ret["scene"][1][0, 0]:.2f})',(5, 14 ), 14)
            im = insert_text(im,f'{imgnet_labels[ret["infer"][2][t, 0]]} ({ret["infer"][1][t, 0]:.2f})',(790, 14), 14)
            im = insert_text(im,f'{imgnet_labels[ret["real"][2][t, 0]]} ({ret["real"][1][t, 0]:.2f})',(1050, 14), 14)
        else:
            im = expand_boarder(im, (0, 54, 0, 0))
            i = 0
            for attacked_name in attacked_names:
                ...
                im = insert_text(im,
                                 ...
                                 (5, 14 * i), 13)
                im = insert_text(im,
                                 f'{attacked_name + " : "+ imgnet_labels[ret["infer" + attacked_name][2][t, 0]]} (p={ret["infer" + attacked_name][1][t, 0]:.2f})',
                                 (790, 14 * i), 13)
                im = insert_text(im,
                                 f'{attacked_name + " : "+ imgnet_labels[ret["real" + attacked_name][2][t, 0]]} (p={ret["real" + attacked_name][1][t, 0]:.2f})',
                                 (1050, 14 * i), 13)
        ...
        # im = insert_text(im, f'L2={warped_prj_l2:.2f}', (630, 14), 14)


        # im = insertText(im, 'SSIM:{:.2f}'.format(pred_ssim), (715, 14), 14)
        # im = insert_text(im, f'L2={pred_dE :.2f}', (730, 14), 14)
...
def summarize_single_attacker(attacker_name, data_root, setup_list, device='cuda', device_ids=[0], pose = 'original'):
...
def summarize_single_attacker_for_UAP_task(attacker_name, data_root, setup_list, device='cuda', device_ids=[0],pose='original'):
    # given the attacker_name and setup_list, summarize all attacks, create stats.txt/xls and montages
    ...
    for setup_name in setup_list:
        for attacker_name in attacker_name_list:
            ...
            # print results
            print(f'\n-------------------- [{attacker_name}] results on [{setup_name}] for USPAA --------------------')
            print(table.to_string(index=False, float_format='%.4f'))
            print('-------------------------------------- End of USPAA result table ---------------------------\n')
            # print(table.filter(regex = 'top-[0-9]_', axis = 1).to_string(index = False, float_format = '%.2f'))  # columns that only contain success rates
            # print(table.filter(regex = '_L2'    , axis = 1).to_string(index = False, float_format = '%.4f'))  # columns that only contain L2
            # print(table.filter(regex = '_dE'    , axis = 1).to_string(index = False, float_format = '%.4f'))  # columns that only contain dE
...
def summarize_all_attackers(attacker_names, data_root, setup_list, recreate_stats_and_imgs=False):
    ...
    pivot_table = pd.pivot_table(table,
                                 ...
                                 values=['T.top-1_real', 'T.top-5_real', 'U.top-1_real', 'T.real_L2', 'T.real_Linf',
                                         'T.real_dE', 'T.real_SSIM', 'All.real_L2', 'All.real_Linf', 'All.real_dE',
                                         'All.real_SSIM'],
