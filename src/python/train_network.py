...
def load_poses_data(data_root, setup_name, index, input_size=None, compensation=False):
    # data paths
    setup_path = join(data_root, 'setups', setup_name)
    cam_ref_path = join(setup_path, f'cam/poses/raw{index}/ref')
    cam_train_path = join(setup_path, f'cam/poses/raw{index}/train')
    cam_valid_path = join(setup_path, f'cam/poses/raw{index}/test')
    cam_cb_path = join(setup_path, f'cam/poses/raw{index}/cb')
    ...
    gray_idx = 1  # cam/poses/raw/ref{index}/img_0002, lit by prj_brightness, cam-captured surface i.e., s when img_gray projected
    cam_scene = cam_ref[gray_idx].unsqueeze(0)  # ref/img_0002 is

    print("Loading data from '{}'".format(cam_train_path))
    # textured train
    ...
    prj_valid = ut.torch_imread_mt(prj_valid_path, index=[i for i in range(
        cam_valid.shape[0])])  # only read the same number of cam_valid
...
def train_pcnet(model, train_data, valid_data, cfg):
    ...
    if cfg.pose_index > -1:
        title = ut.opt_to_string_for_poses(cfg)
...
def get_model_train_cfg(model_list, data_root=None, setup_list=None, device_ids=[0], center_crop=False, load_pretrained=False, plot_on=True,
    ...
    cfg_default.pose_index         = pose_index
    ...
    if single:                     # single model, no list
        ...
        cfg_default.model_name = 'mul_PCNet'
...
def train_eval_pcnet(cfg_default, poses=False, index = -1):
    data_root = cfg_default.data_root
    cfg_default.pose_index= index
    ...
    for setup_name in cfg_default.setup_list:
        ...
        # if multiple poses, load the data for the current pose (not used in CAPAA)
        if poses:
            cam_scene, cam_train, cam_valid, prj_train, prj_valid, cam_mask, mask_corners, setup_info = load_poses_data(
                data_root, setup_name, index)
            ...
            for model_name in cfg_default.model_list:
                ...
                for loss in cfg_default.loss_list:
                    ...
                    if not cfg.load_pretrained:
                        ...
                        if poses:
                            checkpoint_filename = join(data_root, '../checkpoint', ut.opt_to_string_for_poses(cfg) + '.pth')
                ...
                # warping_net.
                grid_fine = pcnet.module.get_warping_grid()
    ...
    # grid_fine = warping_net.module.get_fine_grid()


    return pcnet, ret, cfg, grid_fine
