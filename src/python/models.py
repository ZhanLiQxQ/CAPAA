...
class PCNet(nn.Module):
    ...
    # s is Bx3x256x256 surface image
    # ? s is cam_scene_batch, i think it is Bx3x240x320
    def forward(self, x, s, use_cam = False, adv_attention_map = None, prj_brightness = 0):
        # geometric correction using WarpingNet
        x = self.warping_net(x)

        # 当使用热力图时，将direct_light_mask换成CAM_heatmap
        # for debugging
        if use_cam:
            prj_eraser = torch.ones_like(adv_attention_map) - adv_attention_map
            # 根据CAM热力图权重，用投影灰色图擦除’不重要‘的区域
            x = x * adv_attention_map + prj_brightness * torch.ones_like(adv_attention_map) * prj_eraser
