...
def invertGrid(src_grid, dst_size):
    ...
    # 该函数重新排列了src_grid的维度，并将dst_pts2d（现在src_grid的空间为目标空间dst_grid）重新塑造为一个2xN矩阵（其中N是像素的总数），包含原始网格的(x, y)坐标。
    dst_pts2d = src_grid.permute(3, 1, 2, 0).reshape(2, -1).numpy().transpose()  # data points
    # create_meshgrid生成的值将是图像中每个像素点的 x，y 原坐标，然后归一化，
    # (-1, -1) 代表输出图像的左上角，(1, 1) 代表输出图像的右下角，(0, 0) 代表输出图像的中心。
    ...
    # 使用griddata函数执行线性插值。它使用密集的目标网格将src_pts2d映射到dst_pts2d
