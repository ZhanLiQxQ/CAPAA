...
import time
import yaml
from differential_color_functions import deltaE
...
# You may change to another port
def init_visdom(server='localhost', port=8083):
    import visdom
...
# ------------------------------------------- Data capture -----------------------------------------------
def init_prj_window(prj_w, prj_h, val, offset):
    """
...
def init_cam(cam_raw_sz=None):
    if sys.platform == 'win32':
        cam = cv.VideoCapture(1, cv.CAP_DSHOW)  # windows only to get rid of the annoying warning
    else:
...
def preview_cam(cam_raw_sz=None, cam_crop_sz=None, cam=None):
...
def project_capture_data(prj_input_path, cam_cap_path, setup_info,cam):
