"""
Code obtrained from https://github.com/jacobgil/pytorch-grad-cam
Jacob Gildenblat and contributors, "PyTorch library for CAM methods", Github 2021
"""
...
def resize_to_original(img, cam):
    """
    Args:
        img: (Tensor) shape => (B, C, H, W)
        cam: (Tensor) shape => (1, 1, H', W')
    Return:
        synthesized cam (Tensor): shape =>(3, H, W)
    """
    ...
    cam = cam.squeeze()
    cam = torch.stack([cam] * 3, dim=0)  # (3 * 240 * 320)
    return cam
